...
def online_server(request):
    ...
    for i in unique_item:
        ...
        result = PoemData.objects.filter(poemId=i).values()
        print("orm->result", list(result))
        poemIds.append(i)
    ...
    for _dict in features:
        poemStar, poemDynasty, poemTagNames = _dict["poemStar"], _dict["poemDynasty"], _dict["poemTagNames"]
        try:
